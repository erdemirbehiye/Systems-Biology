...
A = np.ndarray((len(t)))
S = np.ndarray((len(t)))
F = np.ndarray((len(t))) S_floor=np.ndarray((len(t))) X_v = np.ndarray((len(t))) Y_v = np.ndarray((len(t))) R_v = np.ndarray((len(t))) Z_v = np.ndarray((len(t))) Y_dist = np.ndarray((len(t))) Z_dist = np.ndarray((len(t)))
alpha_X=1.0
alpha_Y=1.0
alpha_A=1.0
alpha_S=1.0
...
beta_X_v=speed
 beta_Y_v=speed
beta_R_v=speed
beta_Z_v=speed beta_Y_dist=floor_distance*alpha_Y_dist*floor_choice
if floor_choice==1:
  constant=3.4
elif floor_choice==2:
  constant=4.3
...
c_D=c_X*c_H
dYdt=beta_Y*(c_D**n_D2Y)/(k_D2Y**n_D2Y+c_D**n_D2Y)- alpha_Y * c_Y dAdt = beta_A*(c_Y**n_Y2A)/(k_Y2A**n_Y2A+c_Y**n_Y2A)- alpha_A * c_A c_S = 1.0
#c_S=1.0*((ct >= t_end-10).astype(float)) #stop button
dFdt= beta_F
*(1/((1+(c_S/k_F)**n_F)))*(c_X**n_F/(k_F**n_F+c_X**n_F))- alpha_F * c_F
dS_floordt=beta_S_floor*(c_X**n_X2S_floor)/(k_X2S_floor**n_X2S_floor+c_ X**n_X2S_floor)- alpha_S_floor * c_S_floor
...
nA = c_A + dt * dAdt
nF = c_F + dt * dFdt
n_S_floor = c_S_floor + dt * dS_floordt n_X_v = c_X_v + dt * dX_vdt
n_Y_v = c_Y_v + dt * dY_vdt
n_R_v = c_R_v + dt * dR_vdt
n_Z_v = c_Z_v + dt * dZ_vdt n_Y_dist = c_Y_dist + dt * dY_distdt n_Z_dist = c_Z_dist + dt * dZ_distdt
